...
class Localization():
    def __init__(self):
        ...
        self.location_pub = rospy.Publisher('ekf_w8',Odometry,queue_size=3)
        self.location_pub1 = rospy.Publisher('ekf_w9',Odometry,queue_size=3)
    ...
    def laserCallback(self,msg):
        ...
        t2 = rospy.get_time()
        print "T is : ", T
        print "self.xOdom is: ", self.xOdom
        #test ekf
        ...
        self.publishResult()
        pass
        print "odometry: ", t2-t1
        print "observation: ", t4-t3
        print "ekf: ",t6-t5
    ...
    def publishResult(self):
        ...
        self.odom_broadcaster.sendTransform((s[0],s[1],0.001),(q[0],q[1],q[2],q[3]),
                            rospy.Time.now(),"ekf_w8","world_base")
        # odom
        odom = Odometry()
        ...
        q = tf.transformations.quaternion_from_euler(0,0,s[2])
        self.odom_broadcaster.sendTransform((s[0],s[1],0.001),(q[0],q[1],q[2],q[3]),
                            rospy.Time.now(),"ekf_w9","world_base")
        # odom
        odom = Odometry()
        odom.header.stamp = rospy.Time.now()
        odom.header.frame_id = "world_base"
        ...
        self.location_pub1.publish(odom)
