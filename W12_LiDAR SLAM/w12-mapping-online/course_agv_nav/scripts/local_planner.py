...
class LocalPlanner:
    ...
    def updateGlobalPose(self):
        ...
        while ind < len(self.path.poses):
            # =====================================
            self.path.poses[ind].header.frame_id = "/map"
        ...
        #lgoal = self.tf.transformPose('/map', goal)
        self.midpose_pub.publish(goal)
    ...
    def planOnce(self):
        ...
        # Update obstacle
        self.updateObstacle()
        u, trajectory = dwa_control(self.plan_x,self.plan_config,self.plan_goal,self.plan_ob)
        alpha = 0.5
        print "u is: ", u
        print "Before the computation, vx is: ", self.vx, "vw is: ", self.vw
