...
class Localization():
    ...
    def laserCallback(self, msg):
        ...
        delta_yaw = math.atan2(T_odom[1,0],T_odom[0,0])
        s = self.xOdom
        self.xOdom[0] = s[0] + math.cos(s[2])*T_odom[0,2] - math.sin(s[2])*T_odom[1,2]
        self.xOdom[1] = s[1] + math.sin(s[2])*T_odom[0,2] + math.cos(s[2])*T_odom[1,2]
        ...
        delta_yaw = math.atan2(T_observation[1,0],T_observation[0,0])        
        s = self.xEst
        # z is the self.new
        u = [0, 0, 0]
        u[0] = s[0] + math.cos(s[2])*T_observation[0,2] - math.sin(s[2])*T_observation[1,2]
        u[1] = s[1] + math.sin(s[2])*T_observation[0,2] + math.cos(s[2])*T_observation[1,2]
        u[2] = s[2] + delta_yaw


        self.xEst, self.PEst = self.ekf.estimate(self.xEst, self.PEst, u, T_odom)
        self.publishResult()
