...
import math
import time
class Tracking: # Subscriber -> pathCallback() -> initTracking()/ -> updateGlobalPose
    def __init__(self):
    ...
    def planOnce(self): # compute the velocity
        self.lock.acquire()
        self.updateGlobalPose()
        target = self.path.poses[self.goal_index].pose.position # mid_target
        dx = target.x - self.x  # compute distance
        dy = target.y - self.y
        target_angle = math.atan2(dy, dx)   # beta
        # self.vx == data.linear.x, self.vw == data.angular.z
        self.vx = 0.2
